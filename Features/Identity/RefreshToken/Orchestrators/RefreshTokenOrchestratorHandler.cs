...
public class RefreshTokenOrchestratorHandler : IRequestHandler<RefreshTokenOrchestratorRequest, RequestResponse<string>>
{
    ...
    public async Task<RequestResponse<string>> Handle(RefreshTokenOrchestratorRequest request, CancellationToken cancellationToken)
    {
        ...
        if (tokenEntity == null)
        {
            return RequestResponse<string>.Fail("Invalid refresh token.", 401);
        }

        if (tokenEntity.IsRevoked)
        {
            return RequestResponse<string>.Fail("Refresh token has been revoked.", 401);
        }
        ...
        if (tokenEntity.IsUsed)
        {
            ...
            return RequestResponse<string>.Fail("Refresh token has already been used. Please log in again.", 401);
        }

        if (tokenEntity.ExpiresAt <= DateTime.UtcNow)
        {
            return RequestResponse<string>.Fail("Refresh token has expired. Please log in again.", 401);
        }
        ...
        {
            return RequestResponse<string>.Fail("User account is inactive or not found.", 401);
        }
        ...
        // ─── Phase 5: Return Primitive JWT String ─────────────────────────
        return RequestResponse<string>.Ok(newAccessToken, "Token refreshed successfully.");
    }
}
