...
public class LoginOrchestratorHandler : IRequestHandler<LoginOrchestratorRequest, RequestResponse<LoginDto>>
{
    ...
    public async Task<RequestResponse<LoginDto>> Handle(LoginOrchestratorRequest request, CancellationToken cancellationToken)
    {
        ...
        if (user == null)
        {
            return RequestResponse<LoginDto>.Fail("Invalid email or password.", 400);
        }

        // AC4: Lockout check (consecutive failed attempts lockout window)
        if (user.LockoutEnd.HasValue && user.LockoutEnd.Value > DateTime.UtcNow)
        {
            return RequestResponse<LoginDto>.Fail("Account is locked due to multiple failed login attempts. Please try again later.", 400);
        }
        ...
        {
            return RequestResponse<LoginDto>.Fail("Account is pending verification. Please verify your email before logging in.", 400);
        }

        if (user.AccountStatus != AccountStatus.Active)
        {
            return RequestResponse<LoginDto>.Fail("Account is not active.", 400);
        }
        ...
        // ─── Phase 6: Return DTO ──────────────────────────────────────────
        var loginDto = new LoginDto(accessToken, "Bearer", 900);
        return RequestResponse<LoginDto>.Ok(loginDto, "Login successful.");
    }
}
